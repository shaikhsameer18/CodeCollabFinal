...
const ROOM_FILE_PREFIX = 'room_files_'
...
function FileContextProvider({ children }: { children: ReactNode }) {
    ...
    // Room-specific localStorage key
    const roomFileKey = `${ROOM_FILE_PREFIX}${currentUser?.roomId || 'default'}`

    // Try to load saved file structure from localStorage
    const loadSavedFileStructure = () => {
        try {
            const savedData = getItem(roomFileKey)
            if (savedData) {
                return JSON.parse(savedData)
            }
        } catch (error) {
            console.error('Error loading saved file structure:', error)
        }
        return initialFileStructure
    }

    const [fileStructure, setFileStructure] = useState<FileSystemItem>(loadSavedFileStructure)
    ...
    const collapseDirectories = () => {
        ...
        setFileStructure((prevFileStructure) => {
            const newStructure = collapseDir({...prevFileStructure});
            // Make sure we don't collapse the root directory
            return {
                ...newStructure,
                isOpen: true
            };
        ...
        // Show notification to indicate action was performed
        toast.success("All directories collapsed", { duration: 2000 });
    }
    ...
    const handleFileStructureSync = useCallback(
        ({
            ...
            if (fileStructure) {
                ...
                // Save to localStorage to preserve across refreshes
                if (currentUser?.roomId) {
                    setItem(roomFileKey, JSON.stringify(fileStructure))
                }
            }
            ...
            // Show a less distracting notification
            toast.dismiss()
            toast.success("Files synced", { duration: 1500 })
        },
        [currentUser?.roomId, roomFileKey, setItem],
    )
